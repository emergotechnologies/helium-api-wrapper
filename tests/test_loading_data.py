...
def test_challenge_loading_triangulation():
    ...
    for key in column_types:
        ...
        assert test_df[key].dtype == column_types[key], f"{key} not of {column_types[key]}"

    # TESTING NUMBER OF CHALLENGEES
    # using both lat. and lon. on the off chance that two hotspots share one but not both
    assert len(test_df[['challengee']].drop_duplicates()) == 1, "Wrong number of challengees"
    ...
    assert len(test_df[['challengee', 'witness']].drop_duplicates()) == 3, "Wrong number of witnesses"
